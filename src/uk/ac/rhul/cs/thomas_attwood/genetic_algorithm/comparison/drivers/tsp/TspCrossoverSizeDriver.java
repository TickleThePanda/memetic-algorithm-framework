...
import uk.ac.rhul.cs.thomas_attwood.genetic_algorithm.solutions.tsp.SymmetricTspFunction;
import uk.ac.rhul.cs.thomas_attwood.genetic_algorithm.solutions.tsp.SymmetricTspFunctionFactory;
...
import uk.ac.rhul.cs.thomas_attwood.genetic_algorithm.solutions.tsp.TspTwoOpt;

public final class TspCrossoverSizeDriver {

  private TspCrossoverSizeDriver() {
  }
  ...
  public static void main(final String[] args) {
    ...
    final SymmetricTspFunctionFactory funcFactory =
        new SymmetricTspFunctionFactory(PROBLEM_SIZE);

    for (int j = 0; j < N_PROBLEMS; j++) {
      final SymmetricTspFunction masterfunction =
      ...
      for (int currentCrossover = SMALLEST_CROSSOVER; currentCrossover < SMALLEST_CROSSOVER
        ...
        final SymmetricTspFunction copyFunction = masterfunction.copy();

        final TspTourFactory solFactory1 = new TspTourFactory(copyFunction);

        final TspTwoOpt twoOpt =
            new TspTwoOpt(LocalImprovementType.SINGLE_RANDOM);

        final EvoAlgorithmSettings settings =
            new EvoAlgorithmSettings(POOL_SIZE, ELITE_SIZE, currentCrossover,
  ...
  private static final int STEPS_DUPLICATES = 1000000;
