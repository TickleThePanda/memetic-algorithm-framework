...
public class PsuedoBooleanFunction implements Problem<BitString>, Iterable<PseudoBooleanTerm> {
  ...
  LoadingCache<BitString, Integer> fitnessCache = CacheBuilder.newBuilder()
      ...
      .concurrencyLevel(DEFAULT_CONCURRENCY_LEVEL).build(new CacheLoader<BitString, Integer>() {
  ...
  /**
   * Creates a new PbFunction using the terms provided and enabling the hash depending on
   * <code>doHash</code>.
   * 
   * @param doHash
  ...
  public PsuedoBooleanFunction copy() {
    return new PsuedoBooleanFunction(pseudoBooleanTerms);
  }
  ...
  public String toString() {
    final int maxLen = 3;
    return "PBFitFunc [terms=" + (pseudoBooleanTerms != null
        ? Arrays.asList(pseudoBooleanTerms).subList(0, Math.min(pseudoBooleanTerms.length, maxLen))
        : null) + "]";
  }
