...
public class InsertLocalImprovement implements LocalImprovement<Tour> {
  ...
  public Tour doLazySingleSwap(final Tour solution, final int index1, final int index2) {
    ...
    final int city21 = solution.get(index2);
    final int city23 = solution.get(afterIndex2);

    // distance (city11 -> city12 -> city13) + distance (city21 -> city22)
    final int oldDistance = func.getDistanceBetweenCities(city11, city12)
        + func.getDistanceBetweenCities(city12, city13)
        + func.getDistanceBetweenCities(city21, city23);

    // distance (city11 -> city13) + distance (city21 -> city12 -> city22)
    final int newDistance = func.getDistanceBetweenCities(city11, city13)
        + func.getDistanceBetweenCities(city21, city12)
        + func.getDistanceBetweenCities(city12, city23);
  ...
  /**
   * Reverses order of cities of the solution, between (and including) firstIndex and secondIndex.
   * 
   * @param solution
   *          the solution to be swapped
   * @param index1
  ...
  public Tour doSingleSwap(final Tour solution, final int index1, final int index2) {
    final List<Integer> newOrder = new ArrayList<>(solution.size());
    newOrder.addAll(solution.subTour(0, index1 - 1));
    newOrder.addAll(Lists.reverse(solution.subTour(index1, index2)));
    newOrder.addAll(solution.subTour(index2, solution.size() - 1));
