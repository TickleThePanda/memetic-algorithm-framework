 ...
 * EvolutionaryAlgorithm}. It is one of the core classes of this project. It optimises a solution
 * without a step of local improvement.
 *
 * @param <E>
...
public class GeneticAlgorithm<E extends Solution<E>> implements EvoAlgorithm<E> {
   ...
   * {@link uk.ac.rhul.cs.thomas_attwood.genetic_algorithm.solutions.Generator SolutionFactory}. The
   * seed is to generate random numbers for selection.
   *
   * @param settings
   ...
   * {@link uk.uk.ac.rhul.cs.thomas_attwood.genetic_algorithm.algorithms.EvoAlgorithmSettings
   * EvoAlgorithmSettings}, and the
   * {@link uk.ac.rhul.cs.thomas_attwood.genetic_algorithm.solutions.Generator SolutionFactory}.
   *
   * @param seed
   ...
   *          the solution factory to use to generate and crossover solutions
   */
  public GeneticAlgorithm(final long seed, final EvoAlgorithmSettings settings,
      final Solution.Generator<E> solutionFactory) {
  ...
  /**
   * Constructs a new GeneticAlgorithm with the seed, the
   * {@link uk.ac.rhul.cs.thomas_attwood.genetic_algorithm.solutions.Generator SolutionFactory}, and
   * the default
   * {@link uk.uk.ac.rhul.cs.thomas_attwood.genetic_algorithm.algorithms.EvoAlgorithmSettings
   * EvoAlgorithmSettings}. The seed is to generate random numbers for selection.
   *
   * @param seed
   *          the seed that the algorithm uses for selection
   * @param solutionFactory
   *          the solution factory to use to generate and crossover solutions
   */
  public GeneticAlgorithm(final long seed, final Solution.Generator<E> solutionFactory) {
   ...
   * (non-Javadoc)
   * 
   * @see uk.ac.rhul.cs.thomas_attwood.genetic_algorithm.EvoAlgorithm#doGenerationStep ()
   */
  @Override
  public void doAlgorithmStep() {
    ...
    if (this.countToRemoveDuplicates == 0) {
      ...
      this.countToRemoveDuplicates = this.getSettings().getStepsBeforeRemDups();
  ...
  /**
   * Replaces the population with a mutated population, without damaging the elite population.
   */
  ...
  private void removeDuplicates() {
    ...
    for (int i = this.population.size(); i < this.getSettings().getPoolSize(); i++) {
      this.population.add(this.solutionFactory.generateSolution());
    }
  }

  /**
   * select a parent from the population using a weighted distribution, where the weight is the
